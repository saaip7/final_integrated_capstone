...
#else
    // ========================================================================
    // NORMAL MODE: Timed backward movement with sensor detection
    // ========================================================================
    const float THRESHOLD = 20.0f;      // cm - safety threshold (raised from 10cm)
    const int16_t BACKWARD_SPEED = 20;  // 20% PWM for backward movement
    const uint32_t MOVE_DURATION = 2500;  // Move backward for 2.5 seconds
    const uint32_t STOP_DURATION = 2000;  // Stop for 2 seconds

    printf("\r\n=== TIMED MOVEMENT MODE ===\r\n");
    printf("Backward Speed: %d%%\r\n", BACKWARD_SPEED);
    printf("Movement Pattern: MOVE %lums → STOP %lums → repeat\r\n", MOVE_DURATION, STOP_DURATION);
    printf("Emergency Stop: Sensor A AND B detect obstacle < %.1f cm\r\n\n", THRESHOLD);

    while (1) {
        // ============ PHASE 1: MOVE BACKWARD (2.5 seconds) ============
        printf("\r\n--- MOVING BACKWARD ---\r\n");
        uint32_t move_start = HAL_GetTick();

        while ((HAL_GetTick() - move_start) < MOVE_DURATION) {
            ...
            if (a_detect && b_detect) {
                // BOTH sensors detect obstacle - EMERGENCY STOP
                Motor_Stop_All();
                printf("EMERGENCY STOP! A:%.1f cm, B:%.1f cm - BOTH DETECT OBSTACLE\r\n",
                       sensor_a, sensor_b);
            ...
            // Safe to move - set motor speed (negative for backward)
            Motor_SetSpeed(MOTOR_1, -BACKWARD_SPEED);
            Motor_SetSpeed(MOTOR_2, -BACKWARD_SPEED);
            Motor_SetSpeed(MOTOR_3, -BACKWARD_SPEED);
            Motor_SetSpeed(MOTOR_4, -BACKWARD_SPEED);

            printf("MOVING BACKWARD - A:%.1f cm, B:%.1f cm (Elapsed: %lu ms)\r\n",
                   sensor_a, sensor_b, HAL_GetTick() - move_start);

            HAL_Delay(100);  // Small delay for sensor stability
        }
        ...
        while ((HAL_GetTick() - stop_start) < STOP_DURATION) {
            // Even during stop, check sensors for obstacle detection
            HC_SR04_Trigger_All();
            HAL_Delay(50);

            float sensor_a = HC_SR04_Calculate_Distance(&sensors[0]);
            float sensor_b = HC_SR04_Calculate_Distance(&sensors[1]);

            printf("STOPPED - A:%.1f cm, B:%.1f cm (Stop time: %lu ms)\r\n",
                   sensor_a, sensor_b, HAL_GetTick() - stop_start);

            HAL_Delay(450);  // Total 500ms per check during stop phase
        }

        printf("Stop phase complete. Restarting movement cycle...\r\n");
    }
#endif
